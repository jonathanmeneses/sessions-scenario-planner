...
const TherapyCalculator = () => {
  ...
  const [selectedMetric, setSelectedMetric] = useState<
    keyof typeof METRIC_LABELS | null
  >(null);
  ...
  const calculateBlendedRate = (useAdjusted = true) => {
    const totalSessions = rows.reduce(
      (sum, row) =>
        sum + (useAdjusted ? row.adjustedSessions : row.baseSessions),
      0,
    );
    return totalSessions === 0
      ? 0
      : calculateRevenue(useAdjusted) / totalSessions;
  };
  ...
  const prepareTimeData = (rows: TherapyRow[]) => {
    ...
    const timeByService = rows.reduce(
      (acc, row) => {
        ...
        return {
          Actual: {
            ...acc.Actual,
            [row.visitType]:
              (acc.Actual?.[row.visitType] || 0) + actualTherapy + actualAdmin,
          },
          Planned: {
            ...acc.Planned,
            [row.visitType]:
              (acc.Planned?.[row.visitType] || 0) +
              plannedTherapy +
              plannedAdmin,
          },
        };
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-4 p-4">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link
          href="/simple-widget"
          className="advanced-link inline-flex items-center text-sm text-gray-600 hover:text-blue-700"
        >
        ...
        <CardHeader>
          <div className="flex items-start justify-between">
            <h3 className="text-lg font-semibold">
              {hasEnteredName
                ? `${userName}'s Therapy Practice Calculator`
                : 'Therapy Practice Calculator'}
            </h3>
            <Button
              variant="ghost"
              size="sm"
        ...
        {showWelcomeCard && (
          <CardContent>
            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
              <h3 className="mb-3 font-medium text-gray-900">Quick Guide</h3>
              <ol className="list-decimal space-y-2 pl-5 text-gray-700">
                <li>
                  Add your current practice visit types and frequencies using
                  the <strong>Add Service</strong> button
                </li>
                <li>
                  Edit the existing visit types using the pencil icon to match
                  your current practice baselines
                </li>
                <li>
                  Set up to 3 specific goals in the{' '}
                  <strong>Target Setting</strong> section
                </li>
                <li>
                  Use the sliders for rate and sessions/month for each visit
                  type to see changes to your practice metrics
                </li>
                <li>
                  View your monthly summary and projected values, compared
                  against your selected targets
                </li>
                <li>
                  Explore charts for deeper insights into your practice metrics
                </li>
              </ol>
            </div>
          </CardContent>
                  ...
                  <div className="grid gap-2">
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row">
                      <Select
                        ...
                        onValueChange={(value) =>
                          setFormData({ ...formData, payer: value })
                        }
                      >
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            adminTime: Number(e.target.value),
                          })
                        }
                      />
                    </div>
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            baseSessions: Number(e.target.value),
                          })
                        }
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <div className="mb-2 flex items-center gap-2">
                      <Input
                        ...
                        onChange={(e) =>
                          handleRateInputChange(row.id, e.target.value)
                        }
                    ...
                    <Slider
                      ...
                      min={0}
                      max={
                        rateRanges[row.id] || Math.max(400, row.baseRate * 2)
                      }
                      step={5}
                  ...
                  <div>
                    ...
                    <div className="mb-2 flex items-center gap-2">
                      <Input
                        ...
                        onChange={(e) =>
                          handleSessionInputChange(row.id, e.target.value)
                        }
                        className="no-spinners w-24"
                        min={0}
                      />
                    ...
                    <Slider
                      ...
                      min={0}
                      max={
                        sessionRanges[row.id] ||
                        Math.min(120, row.baseSessions * 3)
                      }
                      step={1}
                      onValueChange={(value) =>
                        handleSessionChange(row.id, value)
                      }
                      className="my-2"
                    />
                  </div>
            ...
            <table className="w-full min-w-[600px]">
              <thead>
                <tr className="border-b">
                  ...
                  <th className="p-2 text-left">Sessions/Month</th>
                    ...
                    <td className="p-2">
                      <div className="w-32">
                        ...
                        <Slider
                          ...
                          max={
                            rateRanges[row.id] ||
                            Math.max(400, row.baseRate * 2)
                          }
                          step={5}
                          onValueChange={(value) =>
                            handleRateChange(row.id, value)
                          }
                        />
                      </div>
                    </td>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1 sm:gap-2">
                            ...
                            <Button
                              ...
                              className="h-8 w-8 text-red-600 hover:bg-red-50 hover:text-red-700 sm:h-9 sm:w-9"
                              title="Delete Goal"
                            >
                    ...
                    {activeSlot !== null && (
                      <div className="space-y-2">
                        <Select
                          ...
                          onValueChange={(value) =>
                            handleMetricSelect(
                              value as keyof typeof METRIC_LABELS,
                              activeSlot,
                            )
                          }
                          ...
                          <SelectContent>
                            {Object.entries(METRIC_LABELS)
                              .filter(([key]) => {
                                ...
                                if (editingMetricIndex !== null) {
                                  const currentMetric =
                                    goalMetrics[editingMetricIndex].type;
                                  return (
                                    key === currentMetric ||
                                    !goalMetrics.some((m) => m.type === key)
                                  );
                                }
                  ...
                  <span
                    className={
                      goalMetrics.find((m) => m.type === 'revenue')!
                        .comparison === 'atLeast'
                        ? calculateRevenue() >=
                          goalMetrics.find((m) => m.type === 'revenue')!.target
                          ? 'text-green-600'
                          : 'text-red-600'
                        : calculateRevenue() <=
                            goalMetrics.find((m) => m.type === 'revenue')!
                    ...
                    className={
                      goalMetrics.find((m) => m.type === 'visits')!
                        ...
                        ? calculateTotalVisits(rows) >=
                          goalMetrics.find((m) => m.type === 'visits')!.target
                          ? 'text-green-600'
                          : 'text-red-600'
                        : calculateTotalVisits(rows) <=
                            goalMetrics.find((m) => m.type === 'visits')!.target
